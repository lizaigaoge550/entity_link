...
class BaseModel(nn.Module):
    def __init__(self, vocab_size, input_dim, output_dim,
        ...
        self._init_h = nn.Parameter(torch.Tensor(n_layers*2, output_dim).to('cuda')) #默认是bidirectional
        self._init_c = nn.Parameter(torch.Tensor(n_layers*2, output_dim).to('cuda'))
        ...
        if bert_weight:
            self.bert = PretrainedBertEmbedder('checkpoint/bert-base-chinese.tar.gz')
    ...
    def use_bert(self, input, offset):
        ...
        bert_output = self.bert(input, offset, type_id)
        assert bert_output.size(1) == offset.size(1)
        return bert_output
